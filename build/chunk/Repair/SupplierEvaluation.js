...
var _EpsModal2 = __webpack_require__(198);
...
var SupplierEvaluation = function (_Component) {
	_inherits(SupplierEvaluation, _Component);

	function SupplierEvaluation(props) {
		_classCallCheck(this, SupplierEvaluation);

		var _this = _possibleConstructorReturn(this, (SupplierEvaluation.__proto__ || Object.getPrototypeOf(SupplierEvaluation)).call(this, props));

		_this.openLog = _this.openLog.bind(_this);
		_this.createHandler = _this.createHandler.bind(_this);
		_this.gotoAddDevice = _this.gotoAddDevice.bind(_this);
		_this.delDeviceItem = _this.delDeviceItem.bind(_this);
		return _this;
	}

	_createClass(SupplierEvaluation, [{
		key: 'render',
		value: function render() {
			var _this2 = this;

			var content = this.combineContent();
			var combineCost = this.combineCost();
			var orderCost = combineCost.orderCost;
			var data = this.props.supplierevaluation;
			return _react2.default.createElement('div', { className: 'eps-repair-supplier-response equipment' }, _react2.default.createElement('header', { className: 'header', ref: 'header' }, _react2.default.createElement('div', { className: 'header-bg-specail' }, _react2.default.createElement('div', { className: 'header-bg' }), _react2.default.createElement('div', { className: 'header-bg-2' })), _react2.default.createElement('div', { className: 'header-c' }, _react2.default.createElement(_UserCard2.default, { data: _.extend(data, {
					name: data['createBy'],
					remark: data['maintenanceRemarks'],
					time: data['dateAppointment'],
					fileCount: data['fileCount'] || 0,
					uploadPhaseName: data['uploadPhaseName'] || ''
				}) }))), _react2.default.createElement('div', { className: 'eps-repair-supr-content evaluation' }, _react2.default.createElement('div', { className: 'eps-box-wrap' }, _react2.default.createElement('div', { className: 'eps-box' }, _react2.default.createElement('div', { className: 'eps-add-small', onClick: this.gotoAddDevice }, _react2.default.createElement('i', { className: 'icon-add-device-small' }), _react2.default.createElement('font', null, "\u6DFB\u52A0\u8BBE\u5907")))), content, _react2.default.createElement(_LoadMore2.default, { container: 'main-c', data: data['loading'], onEndReached: function onEndReached(e) {
					_this2.onEndReached(e);
				} })), orderCost, _react2.default.createElement('div', { className: 'eps-footer' }, _react2.default.createElement('div', { className: 'eps-btn-wrap' }, _react2.default.createElement('div', { className: 'log-btn', onClick: function onClick(e) {
					return _this2.openLog(e);
				} }, _react2.default.createElement('i', { className: 'icon-log' }), _react2.default.createElement('span', null, "\u6D41\u7A0B\u65E5\u5FD7")), _react2.default.createElement('div', { className: 'eps-btn eps-btn-warning-large', onClick: function onClick(e) {
		...
		value: function componentDidMount() {
			...
			PubSub.subscribe('get:child:select:device', function (evt, data) {
				...
				var modelData = self.props.supplierevaluation;
				var initialList = modelData['list'].concat(data['data']);
				console.log(initialList, "initialList");
				var allList = modelData['list'].concat(data['data']);
				allList = _.uniq(allList, function (i) {
					return i['epsid'];
				});
				var repeatData = _.intersection(initialList, modelData['list']);
				var deviceNames = [];
				_.each(repeatData, function (item, index) {
					deviceNames.push(item.deviceName);
				});
				if (initialList.length > allList.length) {
					(0, _EpsModal2.AlertBase)({
						tip: '设备重复添加！',
			...
			window.onJwNavBtnClick = function (data) {
				if (data['type'] == '4') {
					var modelData = self.props.supplierevaluation;
					(0, _constants.openChart)(modelData['createEid'], modelData['orderNumber'], '供应商评估');
				}
			};
		...
		value: function openLog() {
			var datas = this.props.supplierevaluation;
		...
		value: function createHandler() {
			var self = this;
			var data = _.clone(self.props.supplierevaluation);
			var combineCost = self.combineCost();
			var allMoney = combineCost.orderMoney;
			...
			_.each(data['list'], function (i) {
				if (i['deviceList'] && i['deviceList'].length != 0) {
					var deviceList = _.clone(i['deviceList']);
			...
			for (var i = 0; i < data['equipmentRepairList'].length; i++) {
				var item = data['equipmentRepairList'][i];
			...
			for (var i = 0; i < data['partRepairList']; i++) {
				var _item = data['partRepairList'][i];
				if (_item['partOperate'] == '请选择') {
					error = '请选择配件操作建议！';
					break;
				}
				if (_item['partOperate'] == '2' && (typeof _item['partNumber'] == 'undefined' || _item['partNumber'].length == 0 || _item['partNumber'] == '0')) {
					error = '请输入配件维修数量！';
					break;
				}
			}
			...
			var epsDialog = (0, _EpsModal.MemoDialog)({
				...
				onBtnClick: function onBtnClick(memo, callback) {
					...
					// 	}));
					// return;
					(0, _EpsRequest2.default)('/McdEpsApi/joywok/repair/submitSupplierEvaluate', {
						method: 'POST',
						body: JSON.stringify({
							param: {
								eid: window.eid,
		...
		key: 'gotoAddDevice',
		value: function gotoAddDevice() {
			var url = EpsWebRoot + '/#/repairing/add-edit/equipment';
			jw.pushWebView(url);
		}
		//
		...
		key: 'delDeviceItem',
		value: function delDeviceItem(data) {
			var dispatch = this.props.dispatch;
			var suppliereValuationData = this.props.supplierevaluation;
			var newList = [];
			var pageInfoList = [];
			// let storeScrapList = [];
		...
		value: function combineContent() {
			var self = this;
			var list = this.props.supplierevaluation.list;
			if (list && list.length > 0) {
				return _react2.default.createElement('ul', { className: 'eps-list-card-wrap eps-device-list', ref: 'listWrap' }, _.map(list, function (item) {
					return _react2.default.createElement(_DeviceCard.DeviceCardAssess, { delDeviceItem: self.delDeviceItem, deleteBtnShow: true, orderid: self.props.supplierevaluation['orderNumber'], showCardIcon: true, itemdata: _.extend(_.clone(item), {
		...
		value: function combineCost() {
			...
			var orderCost = '';
			var moneyName = data['list'] && data['list'].length > 1 ? 'evaluation-show-money' : 'money-show';
			if (data.showMoney) {
				orderCost = _react2.default.createElement('div', { className: moneyName }, ' ', _react2.default.createElement(_MoneyShow2.default, { allData: datas }), ' ');
			} else {
				orderCost = '';
			}
...
exports.default = {
	...
	reducers: {
		...
		changeItemData: function changeItemData(state, action) {
	...
	effects: {
		fetch: /*#__PURE__*/regeneratorRuntime.mark(function fetch(_ref, _ref2) {
			...
			var call = _ref2.call,
			    put = _ref2.put,
			    select = _ref2.select;
			var datas, firstData, type, loading, list, allData, allParts, showMoney, maintenancePrice, orderMoney, money;
				...
				while (1) {
					switch (_context.prev = _context.next) {
						...
						case 5:
							...
							type = datas["supplierevaluation"]['type'];
							loading = datas["supplierevaluation"]['loading'];
