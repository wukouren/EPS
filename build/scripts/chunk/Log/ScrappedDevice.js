...
var ScrappedDevice = function (_Component) {
	_inherits(ScrappedDevice, _Component);

	function ScrappedDevice(props) {
		_classCallCheck(this, ScrappedDevice);

		var store = new Store('Joywok:cache:tabs:scrappeddevice');
		var cache = store.find({ id: 'tab:cache' }) || {};
		if (cache['id']) {
			props['scrapped']['parentData'] = cache["data"];
			console.log(cache["data"], '123123123123123123');
		}
		return _possibleConstructorReturn(this, (ScrappedDevice.__proto__ || Object.getPrototypeOf(ScrappedDevice)).call(this, props));
	}

	_createClass(ScrappedDevice, [{
		key: 'render',
		value: function render() {
			var self = this;
			...
			if (data['parentData']['scrappType'] == 'repair') {
				if (data['parentData']['scrappOrderType'] == 'device') {
					...
					_.each(list, function (item) {
						if (item['partName']) {
							var datas = _.findWhere(data['parentData']['deviceList'], { deviceName: item['deviceName'] });
							if (datas) {
								nowList.push(datas);
							}
						}
					...
					var _nowList = [];
					_.each(list, function (item) {
						var datas = _.findWhere(data['parentData']['pageInfo']['list'], { itDeviceName: item['itDeviceName'] });
						if (datas) {
							_nowList.push(datas);
						}
					});
			...
			return _react2.default.createElement('div', { className: 'root-container' }, _react2.default.createElement('div', { className: 'root-container-w' }, _react2.default.createElement('header', { className: 'header specail', ref: 'header' }, _react2.default.createElement('div', { className: 'header-bg' }), _react2.default.createElement('div', { className: 'header-bg-2' }), _react2.default.createElement('div', { className: 'header-c' })), _react2.default.createElement('sesstion', { className: 'main' }, _react2.default.createElement('div', { className: 'main-c' }, list && list.length > 0 ? _.map(list, function (i, index) {
				return _react2.default.createElement(_ScrappedCard.ScrappedCardSearchCheck, { scrappType: data['parentData']['scrappType'], scrappOrderType: data['parentData']['scrappOrderType'], itemdata: i, index: index + 1 + '/' + list.length, choseScrappedDevice: function choseScrappedDevice(e) {
						return self.choseScrappedDevice(i, index);
					} });
			}) : _react2.default.createElement(_EmptyView2.default, { tip: "\u6682\u65E0\u53EF\u6311\u9009\u8D44\u4EA7" })))));
		}
	}, {
		key: 'choseScrappedDevice',
		value: function choseScrappedDevice(data) {
	...
	return ScrappedDevice;
}(_react.Component);

function mapStateToProps(state) {
	return state;
}
exports.default = (0, _dva.connect)(mapStateToProps)(ScrappedDevice);
