...
import { VendorCardSelect, VendorCardShow } from './../../components/Project/VendorCard';
...
class PMConfirm extends Component {
  ...
  rejectOrder(approveFlg, memo, callback) {
    ...
    // return;
    request('/McdEpsApi/joywok/project/submitOrderInfo', {
      method: 'POST',
      body: JSON.stringify({
        param: {
  ...
  componentDidMount() {
    ...
    //获取供应商列表
    request('/McdEpsApi/joywok/project/getVendorInfo', {
      method: 'POST',
      body: JSON.stringify({
        param: {
          eid: eid,
    ...
    request('/McdEpsApi/joywok/project/getOrderInfo', {
      method: 'POST',
      body: JSON.stringify({
        param: {
          eid: eid,
  ...
  render() {
    let self = this;
    let data = this.props.pmconfirm;
    let isNotChecked = /view/.test(location.href);//在途和历史的拒绝备注右边的单选框不能选择
    ...
    if (data.loading) {
      ...
      let buttons = '',
        project = data,
        strOrderSta = project['orderState'] && orderStatus["project"]['41'][project['orderState']] ? orderStatus["project"]['41'][project['orderState']] : { 'label': '' };
      ...
      return (
        <div className="eps-nonproject-approval eps-project-pmconfirm">
          ...
          <div className="np-total-price" ref="price">
            ...
            <div className="total-p-text">
              <i className="text-icon"></i>
              在合同期内，如遇增值税税率发生变化，订单项下不含税价保持不变。
          </div>
            {/* <div  className="total-p-cell">
            <label>总价</label>
				   	<span><font>{data.pmtotal?data.pmtotal.toFixed(2):'-'}</font><i className="icon-sprice"></i></span>
