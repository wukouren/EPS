				...
				<ul className="">
					{
						data.list.map((item, key) => (
							<div className={"eps-list-card eps-project animated zoomIn eps-bookmark-wrap"} onClick={() => { self.gotoInfoList(item) }}>
								...
								{!isTsi && <div className="eps-item-info-inline">
									<div className="eps-item-info"><dt><label>总价(¥)</label></dt><dd><font onClick={(e) => { e.stopPropagation(); self.NameInfo(item.priceall + '¥', 6) }}>{this.turnMoney(item.priceall)} ¥</font></dd></div>
								</div>}
								{isTsi && <div className="project-it-text border-line-h before">
									<div className="project-it-cell space-b">
										<div className="project-it-flex w60">麦当劳采购总价(含税)</div>
										<div className="project-it-flex" onClick={(e) => { e.stopPropagation(); self.NameInfo(item.mdcSumCost + '¥', 6) }}>{self.turnMoney(item.mdcSumCost)} ¥</div>
									</div>
									<div className="project-it-cell space-b">
										<div className="project-it-flex w60">TSI采购总价(含税)</div>
										<div className="project-it-flex" onClick={(e) => { e.stopPropagation(); self.NameInfo(item.tsiSumCost + '¥', 6) }}>{self.turnMoney(item.tsiSumCost)} ¥</div>
									</div>
									<div className="project-it-cell space-b">
										<div className="project-it-flex w60">餐厅采购总价(含税)</div>
										<div className="project-it-flex" onClick={(e) => { e.stopPropagation(); self.NameInfo(item.eqPriceAll + '¥', 6) }}>{self.turnMoney(item.eqPriceAll)} ¥</div>
									</div>
									<div className="project-it-cell space-b">
										<div className="project-it-flex w60">人工费(含税)</div>
										<div className="project-it-flex" onClick={(e) => { e.stopPropagation(); self.NameInfo(item.labourCost + '¥', 6) }}>{self.turnMoney(item.labourCost)} ¥</div>
									</div>
									<div className="project-it-cell space-b">
										<div className="project-it-flex w60">差旅费(不含税)</div>
										<div className="project-it-flex" onClick={(e) => { e.stopPropagation(); self.NameInfo(item.travelExpensesNotax + '¥', 6) }}>{self.turnMoney(item.travelExpensesNotax)} ¥ </div>
									</div>
									<div className="project-it-cell space-b">
										<div className="project-it-flex w60">差旅费(含税)</div>
										<div className="project-it-flex" onClick={(e) => { e.stopPropagation(); self.NameInfo(item.travelExpenses + '¥', 6) }}>{self.turnMoney(item.travelExpenses)} ¥ ({item.travelExpensesTax || '-'})</div>
									</div>
									<div className="project-it-cell space-b">
										<div className="project-it-flex w60">特批费用(含税)</div>
										<div className="project-it-flex" onClick={(e) => { e.stopPropagation(); self.NameInfo(item.special + '¥', 6) }}>{self.turnMoney(item.special)} ¥</div>
									</div>
									<div className="project-it-cell space-b">
										<div className="project-it-flex w60">整单总价(含税)</div>
										<div className="project-it-flex" onClick={(e) => { e.stopPropagation(); self.NameInfo(item.priceall + '¥', 6) }}>{self.turnMoney(item.priceall)} ¥</div>
									</div>
								</div>}
							</div>
						))
