...
import HeaderCard from './../../components/NewStoreIT/HeaderCardApproval';
import { DeviceCard, ProjectCard, ITCard } from './../../components/NewStoreIT/ProjectCard';
...
class Approval extends Component {
	...
	submitStoreOrder(memo, approveFlg, callback) {
		...
		request('/McdEpsApi/joywok/newstoreit/submitStoreOrderInfo', {
			method: 'POST',
			body: JSON.stringify({
				param: {
	...
	componentDidMount() {
		...
		request('/McdEpsApi/joywok/newstoreit/getStoreInfo', {
			method: 'POST',
			body: JSON.stringify({
				param: {
					eid: userinfo.employee_id,
	...
	render() {
		...
		let updata = decodeURIComponent(decodeURIComponent(decodeURIComponent(this.props.location.query.updateDate)));
		...
		if (data.loading.loading) {
			...
			if (typeof (data['list'][0]) == 'object') {
				...
				headerBox = (<ITCard isTsi={data["isTsi"]} orderid={this.props.params.orderid} updateDate={encodeURIComponent(updata)} itemdata={data['list'][0]} animated={false} viewmore={true} />);
