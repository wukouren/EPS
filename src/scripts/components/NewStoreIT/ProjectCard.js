		...
		if (this.props.viewmore == true) {
			viewmore = (<div className="todo-card">
				<div
					...
					onClick={(e) => {
						e.stopPropagation();
						openWebView('/newstoreit/info-list/equipment/' + this.props.orderid)
					}}
				>查看更多明细</div>
			</div>);
		}
		...
		if (this.props.viewmore == true) {
			viewmore = (<div className="todo-card">
				<div
					...
					onClick={(e) => {
						e.stopPropagation();
						openWebView('/newstoreit/info-list/project/' + this.props.orderid)
					}} >查看更多明细</div>
			</div>);
		}
		return (
		...
		if (this.props.viewmore == true) {
			viewmore = (<div className="todo-card">
				<div
					...
					onClick={(e) => {
						e.stopPropagation();
						openWebView('/newstoreit/info-list/it/' + this.props.orderid + '?updateDate=' + self.props.updateDate)
		...
		return (
			<div className={this.props.animated == false ? "eps-list-card eps-project" : "eps-list-card eps-project animated zoomIn"}>
				...
				<div className="eps-list-inline">
					...
					<div className="eps-list-item">
						<label>{this.props.isTsi ? '设备费用' : '价税合计'}</label>
						{/* <label>设备单价</label> */}
						{this.props.isTsi && <font className="ellipsis" onClick={(e) => { e.stopPropagation(); self.NameInfo(data.tsiSumCost + ' ¥', 4) }}>{this.turnMoney(data.tsiSumCost)} ¥</font>}
						{!this.props.isTsi && <font className="ellipsis" onClick={(e) => { e.stopPropagation(); self.NameInfo(data.mdcSumCost + ' ¥', 4) }}>{this.turnMoney(data.mdcSumCost)} ¥</font>}
						{/* <font className="ellipsis" onClick={(e) => { e.stopPropagation(); self.NameInfo(data.purchasePrice + ' ¥', 4) }}>{this.turnMoney(data.purchasePrice)} ¥</font> */}
					</div>
				</div>
				{viewmore}
			</div>
